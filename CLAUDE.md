...
- **Misc** — `peekView.*`, `gitDecoration.*`, `scrollbarSlider.*`, `badge.*`, `editorGroup.border`, `editorIndentGuide.background1`/`activeBackground1` (the unnumbered forms are deprecated — VSCode now uses per-level indexed keys), `editorBracketMatch.*`
- **Editor highlights (not covered by `editor.*` generically)** — `editor.wordHighlightBackground`, `editor.wordHighlightStrongBackground`, `editor.selectionHighlightBackground`. When cursor is on a symbol or text is selected, VSCode highlights matching occurrences using these keys. Omit them and the fallback comes from the active base theme — e.g. a dark base theme's translucent-dark band rendered over our light `editor.background` makes the highlighted text illegible. Reuse the same alpha as `editor.findMatchHighlightBackground` / `editor.selectionBackground` for consistency.
